...
export default function ConstitutionPage() {
  ...
  async function handleSave() {
    if (!form.description.trim()) { setMsg("请填写描述"); return; }
    setMsg("");
    const body = {
      rule_type: form.rule_type || "入会条件",
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="mt-1 text-[14px] text-[#6c777b]">入会条件与筛选标准管理</p>
          </div>
        ...
        {showForm && (
          <div className="mb-6 rounded-[14px] border border-[#dbe6e4] bg-white p-6 shadow-sm">
            <h2 className="text-[16px] font-bold text-[#004f46] mb-4">{editing ? "编辑" : "新建规则"}</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="text-[12px] font-semibold text-[#4a5c5e] block mb-1">类型</label>
                <select className="w-full rounded-[8px] border border-[#dce6e4] bg-[#fdfcfa] px-3 py-2 text-[13px] outline-none focus:border-[#00836f]" value={form.rule_type} onChange={function(e) { setForm({ ...form, rule_type: e.target.value }); }}>
                  ...
                  <option value="入会条件">入会条件</option>
                  <option value="筛选标准">筛选标准</option>
                </select>
              </div>
              <div>
                <label className="text-[12px] font-semibold text-[#4a5c5e] block mb-1">标识</label>
                <input type="text" className="w-full rounded-[8px] border border-[#dce6e4] bg-[#fdfcfa] px-3 py-2 text-[13px] outline-none focus:border-[#00836f]" placeholder="规则标识" value={form.rule_key} onChange={function(e) { setForm({ ...form, rule_key: e.target.value }); }} />
              </div>
            </div>
            <div className="mb-4">
              <label className="text-[12px] font-semibold text-[#4a5c5e] block mb-1">描述</label>
              <input type="text" className="w-full rounded-[8px] border border-[#dce6e4] bg-[#fdfcfa] px-3 py-2 text-[13px] outline-none focus:border-[#00836f]" placeholder="规则描述" value={form.description} onChange={function(e) { setForm({ ...form, description: e.target.value }); }} />
            </div>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="text-[12px] font-semibold text-[#4a5c5e] block mb-1">运算符</label>
                <input type="text" className="w-full rounded-[8px] border border-[#dce6e4] bg-[#fdfcfa] px-3 py-2 text-[13px] outline-none focus:border-[#00836f]" placeholder="运算符" value={form.operator} onChange={function(e) { setForm({ ...form, operator: e.target.value }); }} />
              </div>
              <div>
        ...
        {loading ? (
          <p className="text-center text-[14px] text-[#8ba09c] py-10">加载中...</p>
        ) : rules.length === 0 ? (
          <div className="text-center py-10">
            <p className="text-[48px] mb-2">📜</p>
            <p className="text-[14px] text-[#6c777b]">暂无规则</p>
          </div>
        ) : (
                    ...
                    <tr key={r.id} className="border-b border-[#f0f5f3] hover:bg-[#fafdfc] text-[13px]">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button onClick={function() { openEdit(r); }} className="text-[12px] text-[#00836f] hover:underline">编辑</button>
                          <button onClick={function() { handleDelete(r); }} className="text-[12px] text-red-400 hover:underline">删除</button>
                        </div>
                      </td>
                    </tr>
                  );
