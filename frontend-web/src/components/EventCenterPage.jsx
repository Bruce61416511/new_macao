    ...
    <div className="min-h-screen bg-gradient-to-b from-[#f2f8f5] via-[#f8fbf9] to-[#f0f5f3] pl-[240px]">
      ...
      <header className="sticky top-0 z-30 flex h-[88px] items-center justify-between border-b border-white/60 bg-white/70 px-11 backdrop-blur-2xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-serifCn text-[30px] font-bold leading-none text-[#00473f] tracking-tight">协会活动</h1>
            <p className="mt-0.5 text-[12px] text-[#8ba09c]">{isRoot ? "活动管理 · 创建与维护" : "精彩活动 · 等你参与"}</p>
          </div>
        </div>
      ...
      <div className="px-11 py-8 max-w-[1200px]">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="mt-1 text-[13px] text-[#8ba09c]">
              {events.length === 0 ? "暂未发布活动" : `共 ${events.length} 场活动 · ${openEvents.length} 场开放中`}
            </p>
          </div>
          {isRoot && (
            <button
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="mt-4 text-[14px] font-medium text-[#8ba09c]">加载活动中...</p>
          </div>
        )}
        ...
        {!loading && events.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <h3 className="mt-5 text-[18px] font-bold text-[#3d5a55]">暂无活动</h3>
            <p className="mt-1 text-[14px] text-[#8ba09c]">{isRoot ? "点击上方按钮创建第一个活动" : "敬请期待更多精彩活动"}</p>
          </div>
        )}
        ...
        {openEvents.length > 0 && (
          <div className="mb-10">
            <div className="mb-5 flex items-center gap-3">
              ...
              <h3 className="text-[16px] font-bold text-[#00473f] tracking-tight">开放报名</h3>
            ...
            <div className="w-full max-w-[500px] animate-[scaleIn_0.25s_ease] rounded-2xl border border-[#dde7e5] bg-white p-7 shadow-[0_25px_60px_rgba(0,66,63,0.15)]" onClick={e => e.stopPropagation()}>
              <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-[19px] font-bold text-[#142528]">创建新活动</h2>
                </div>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                <Field label="活动标题" value={form.title} onChange={v => setForm(p => ({ ...p, title: v }))} required placeholder="请输入活动标题" />
                <div className="grid grid-cols-2 gap-4">
                  <Field label="活动时间" type="datetime-local" value={form.event_date} onChange={v => setForm(p => ({ ...p, event_date: v }))} required />
                  <Field label="地点" value={form.location} onChange={v => setForm(p => ({ ...p, location: v }))} required placeholder="活动地点" />
                </div>
                <Field label="活动描述" value={form.description} onChange={v => setForm(p => ({ ...p, description: v }))} placeholder="简要描述活动内容" />
                <div className="grid grid-cols-2 gap-4">
                  <Field label="价格（澳门元）" type="number" value={form.price_normal} onChange={v => setForm(p => ({ ...p, price_normal: v }))} />
                  <Field label="人数上限" type="number" value={form.max_participants} onChange={v => setForm(p => ({ ...p, max_participants: v }))} placeholder="不限则留空" />
                </div>
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-4">
                <span className={`inline-flex items-center gap-1.5 text-[14px] font-bold ${
                  ...
                  {event.price_normal > 0 ? `MOP ${event.price_normal}` : "🆓 免费"}
                </span>
                <span className="inline-flex items-center gap-1.5 text-[13px] text-[#8ba09c]">
                  ...
                  {event.registrations_count || 0}{event.max_participants ? `/${event.max_participants} 人` : " 人已报名"}
                </span>
              </div>
